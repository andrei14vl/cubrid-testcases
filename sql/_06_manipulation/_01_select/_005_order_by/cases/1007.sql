...
SELECT * FROM cte;
WITH cte as (
select   count(*), a,b,(select   c from xoo xx order by xx.order_no asc for orderby_num() =1 ) xx 
...
SELECT * FROM cte;
WITH cte as (
select   count(*), a,b,(select   y.c from xoo y order by y.order_no asc for orderby_num() = 1 ) xx 
from xoo 
...
SELECT * FROM cte;
WITH cte as (
select   a,b from ( select   a,b,order_no from xoo order by order_no asc) x order by order_no asc)
SELECT * FROM cte;
WITH cte as (
select   (select  a from xoo order by a asc for orderby_num() = 1 ) c1 , b  from xoo x order by order_no asc)
SELECT * FROM cte;
WITH cte as (
select   (select  a from xoo order by b asc,a asc for orderby_num() = 1 ) c1 , b  from xoo x order by order_no asc)
SELECT * FROM cte;
WITH cte as (
select   (select  a from xoo order by order_no asc for orderby_num() = 1 ) c1 , b  from xoo x order by order_no asc)
SELECT * FROM cte;
drop xoo;
